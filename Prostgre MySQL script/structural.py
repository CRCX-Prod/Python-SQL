...
postQuery = """select b_structure.data->>'site_id' as "Site ID",
  ...
  CONCAT(b_structure.data->'tower_id' ->>'prefix', '-',b_structure.data->'tower_id' ->>'prefix') as "Tower ID",
  ...
  COALESCE((SELECT value_title FROM cos_mview_terminology_values WHERE id = 64 AND value_id = b_structure.data->>'type_of_structure'), '') as "Structure type",
  ...
  b_structure.data->>'active_wind_zone' as "Wind speed - Active",
